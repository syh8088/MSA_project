...
import com.order_service.feign.TossPaymentFeignConfig;
...
public interface TossPaymentClient {
    ...
    ResponseEntity<String> paymentConfirm(@RequestHeader("Idempotency-Key") String idempotencyKey, @RequestBody final PaymentConfirmCommand request);

    /**
     * <h1>신용카드 - (결제) 승인신청 API - 강제 에러 발생 전용 API</h1> </br>
     **/
    @PostMapping(value = "/v1/payments/confirm", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<String> paymentError(@RequestHeader("Idempotency-Key") String idempotencyKey, @RequestHeader("TossPayments-Test-Code") String testErrorCode, @RequestBody final PaymentConfirmCommand request);
