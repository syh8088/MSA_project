package kiwi.shop.wallet.application.service;

import kiwi.shop.wallet.adapter.out.exception.RetryExhaustedWithOptimisticLockingFailureException;
...
public class SettlementService implements SettlementUseCase {
    ...
    private final InsertWalletTransactionsPort insertWalletTransactionsPort;
    private final GetPaymentOrderPort getPaymentOrderPort;
    private final TransactionTemplate transactionTemplate;

    @Transactional
    @Override
    public void settlementProcess(String orderId) {

        /**
         * 정산이 이미 처리되었는지 여부를 판단 합니다.
         * 이미 처리된 결제 이벤트 메시지라면 월렛 이벤트 메시지를 반환하도록 한다.
         */
        if (getWalletPort.isAlreadyProcessWallet(orderId)) {
...
//            return this.createWalletEventMessage(orderId);
        ...
        List<PaymentOrderWithSellerOutPut> paymentOrders
                = getPaymentOrderPort.selectPaymentOrderListWithSellerByOrderId(orderId);

        if (Objects.isNull(paymentOrders) || paymentOrders.isEmpty()) {
            return;
        }
...
//        return this.createWalletEventMessage(orderId);
    ...
    private void updateWallet(List<WalletOutPut> updatedWallets) {
        transactionTemplate.execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                updateWalletPort.updateWalletList(updatedWallets);
                insertWalletTransactionsPort.insertWalletTransactionList(updatedWallets);
            }
        });
    }
    ...
    private List<WalletOutPut> getUpdatedWallets(Map<Long, List<PaymentOrderWithSellerOutPut>> paymentOrderWithSellerGroupingSellerNoMap) {
        ...
        List<WalletOutPut> wallets = getWalletPort.selectWalletListBySellerNoList(sellerNoList.stream().toList());
    ...
    private void performSaveOperationWithRecent(List<WalletOutPut> requestWallets) {
        ...
        List<WalletOutPut> walletOutPuts = getWalletPort.selectWalletListWithWalletTransactionsByWalletNoList(walletNoList.stream().toList());
